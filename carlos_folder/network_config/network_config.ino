...
int sendMessage() 
{
  ...
  if(!validAddress)
  {
    Serial.println(F("Invalid transmit address.\n\r"));
  }
...
{ 
  ...
  while (!radio.available()) 
  {
    ...
    if(receiveHeader.to_address == myAddresses[1].address){
      ...
      Serial.println(F("This message is not for you, forwarding message"));
    }  
}
...
void choosePower(int POWER)  //Select which power level to run the nRF24L01+ at
{
    if(POWER == 0) 
    {
      ...
      Serial.println(F("***Invalid Power Level. Setting to Default: 0dBm***"));
      radio.setPALevel(RF24_PA_MAX);
    }
}
...
void setupProfile()
{
  ...
  {
    Serial.print(F("Enter a new name. Max 10 characters: "));
    Serial.flush();
    ...
    Serial.println(F("Choose TX Power Level:\n\r 0: -18dBm\n\r 1: -12dBm\n\r 2: -6dBm \n\r 3: 0dBm  \n\r"));
...
void checkConnection() 
{
  ...
  char message[2] = "k";
  bool k = false;
  Serial.println("Inside checkConnection()");
  ...
  for(int i = 0; i < 6; i++){
     ...
     k = radio.write(&message, sizeof(message));
     radio.startListening();
     Serial.println(k);
